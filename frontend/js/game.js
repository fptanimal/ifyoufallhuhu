...
function simulateHealthAnalysis(age, gender, bmi, sleep, sleepBad, dietVal, exVal, risks, job, weightChange, surgery, surgeryDetail, bpVal, bsVal, hrVal, cholVal) {
    var W = window.AI_MODEL_WEIGHTS || { baseLifeMale: 74, baseLifeFemale: 79, bp_penalty: -3, bs_penalty: -4, bmi_penalty: -5, chol_penalty: -2 };
    var baseLife = gender === 'male' ? W.baseLifeMale : W.baseLifeFemale;
    var adj = 0;
    // BMI impact
    if (bmi < 18.5) adj -= 2;
    else if (bmi > 30) adj += W.bmi_penalty; // Use dynamic model weight
    else if (bmi > 25) adj -= 2;
    else adj += 1;
    // Sleep impact
    if (sleep >= 7 && sleep <= 8) adj += 3;
    else if (sleep >= 6) adj += 1;
    else if (sleep < 5) adj -= 4;
    ...
    else if (exVal <= 0) adj -= 3;
    // Objective medical impacts (amplified by AI Model prevalence)
    var bpMod = W.bp_penalty < -3 ? 1.5 : 1;
    var bsMod = W.bs_penalty < -4 ? 1.5 : 1;
    var cholMod = W.chol_penalty < -2 ? 1.5 : 1;

    if (bpVal) adj += (bpVal < 0 ? bpVal * bpMod : bpVal);
    if (bsVal) adj += (bsVal < 0 ? bsVal * bsMod : bsVal);
    if (hrVal) adj += hrVal;
    if (cholVal) adj += (cholVal < 0 ? cholVal * cholMod : cholVal);
    // LỐI SỐNG KÍCH HOẠT ĐỘT QUỴ
    if (risks.indexOf('smoking') > -1) adj -= 8;
    if (risks.indexOf('alcohol') > -1) adj -= 4;
    if (risks.indexOf('stress') > -1) adj -= 4;
    if (risks.indexOf('overwork') > -1) adj -= 3;
    if (risks.indexOf('tam_dem') > -1) adj -= 8; // Tắm đêm cực kỳ nguy hiểm cho tim mạch
    if (risks.indexOf('an_man') > -1) adj -= 5; // Ăn mặn tăng huyết áp
    if (risks.indexOf('caffeine') > -1) adj -= 2;
    if (risks.indexOf('insomnia2d') > -1) adj -= 7; // Thức trắng cày đêm
    if (risks.indexOf('nobreak') > -1) adj -= 4; // Ngồi lì làm tăng nguy cơ thuyên tắc tĩnh mạch
    if (risks.indexOf('sedentary') > -1) adj -= 4;

    // TIỀN SỬ BỆNH LÝ CHUYÊN SÂU TIM MẠCH
    if (risks.indexOf('tim_mach') > -1) adj -= 9;
    if (risks.indexOf('huyet_ap') > -1) adj -= 6;
    if (risks.indexOf('tieu_duong') > -1) adj -= 7;
    if (risks.indexOf('mo_mau') > -1) adj -= 5;
    if (risks.indexOf('roi_loan_nhip_tim') > -1) adj -= 8;
    
    // GIA ĐÌNH
    if (surgery === 'major') {
        adj -= 6; // Tiền sử gia đình có người đột quỵ/nhồi máu
    } else if (surgery === 'minor') {
        adj -= 3;
    }

    // TRIỆU CHỨNG CẢNH BÁO ĐỘT QUỴ/TIM MẠCH (LÂM SÀNG)
    if (risks.indexOf('dau_nguc') > -1) adj -= 8;
    if (risks.indexOf('tuc_nguc_trai') > -1) adj -= 15; // Dấu hiệu điển hình nhồi máu cơ tim
    if (risks.indexOf('te_yeu_nua_nguoi') > -1) adj -= 15; // Dấu hiệu điển hình đột quỵ FAST
    if (risks.indexOf('kho_noi') > -1) adj -= 12; // Dấu hiệu đột quỵ FAST
    if (risks.indexOf('kho_tho') > -1) adj -= 6;
    if (risks.indexOf('tim_dap') > -1) adj -= 5;
    if (risks.indexOf('chong_mat') > -1) adj -= 5;
    if (risks.indexOf('mo_mat') > -1) adj -= 6;
    if (risks.indexOf('dau_dau') > -1) adj -= 5; // Đau đầu dữ dội
    if (risks.indexOf('mat_ngu') > -1) adj -= 5;
    if (risks.indexOf('do_mo_hoi') > -1) adj -= 4;
    
    if (weightChange === 'loss') adj -= 6;
    else if (weightChange === 'gain') adj -= 5;

    // NEW: Job Risks (Dựa trên nhóm nghề)
    var jobL = (job || '').toLowerCase();
    if (jobL.indexOf('lao_dong_nang') > -1) adj -= 4; // Nguy cơ an toàn lao động, hao mòn thể lực
    if (jobL.indexOf('quan_ly') > -1) adj -= 3; // Stress cao, nguy cơ tim mạch và đột quỵ
    if (jobL.indexOf('ky_thuat') > -1) adj -= 2; // Rủi ro làm thêm giờ, tai nạn hoặc ngồi nhiều (CNTT)
    if (jobL.indexOf('nghiep_vu') > -1) adj -= 2; // Ngồi nhiều, nguy cơ bệnh văn phòng
    if (jobL.indexOf('khoa_hoc') > -1) adj -= 1; // Áp lực trí não
    if (jobL.indexOf('nghe_thuat') > -1) adj -= 2; // Sinh hoạt thất thường

    // Cumulative risk penalty: many risks compound
    ...
    // Random Life factors impact on lifespan prediction
    ...
    var lifespan_base = baseLife + adj + (Math.random() - 0.5) * 4;
    // Nếu tuổi hiện tại bằng hoặc hơn tuổi thọ cơ bản, thưởng thêm thời gian sống phù hợp
    if (age >= lifespan_base - 5) {
        lifespan_base = age + Math.max(1, 5 + (adj / 2));
    }

    var lifespan_min = Math.round(Math.max(30, lifespan_base - (2 + Math.random() * 2)));
    var lifespan_max = Math.round(lifespan_base + (2 + Math.random() * 3));

    // Luôn đảm bảo tuổi thọ tối thiểu không nhỏ hơn tuổi hiện tại (Tối đa của loài người đặt là 120)
    ...
    var riskPct = Math.round(Math.max(5, Math.min(98, 50 - adj * 2.5 + (Math.random() - 0.5) * 10)));
...
function showAIResult(r) {
    ...
    if (typeof MEDICAL_DATA !== 'undefined') {
        ...
        var sourceLinks = [
            { icon: '🏛', name: 'GSA 2025 — Hội nghị Đột quỵ Toàn cầu', url: 'https://www.world-stroke.org' },
            { icon: '🏥', name: 'BV Bạch Mai — Trung tâm Đột quỵ', url: 'https://bachmai.gov.vn' },
            { icon: '🇻🇳', name: 'Bộ Y tế Việt Nam', url: 'https://moh.gov.vn' },
            { icon: '🌍', name: 'WHO — Tổ chức Y tế Thế giới', url: 'https://www.who.int/vietnam' },
            { icon: '🇺🇸', name: 'CDC — Trung tâm Kiểm soát Bệnh tật Hoa Kỳ', url: 'https://www.cdc.gov/stroke' },
            { icon: '❤️', name: 'AHA — Hiệp hội Tim mạch Hoa Kỳ', url: 'https://www.heart.org' },
            { icon: '📰', name: 'WSO — Tổ chức Đột quỵ Thế giới 2024', url: 'https://www.world-stroke.org/world-stroke-day' },
            { icon: '📖', name: 'The Lancet 2021 — Overwork & Stroke', url: 'https://www.thelancet.com' },
            { icon: '🧬', name: 'NEJM — New England Journal of Medicine', url: 'https://www.nejm.org' },
            { icon: '🧠', name: 'NIH — Viện Sức khỏe Quốc gia Hoa Kỳ', url: 'https://www.nih.gov' },
            { icon: '💊', name: 'ACC/AHA 2023 — Hướng dẫn Huyết áp', url: 'https://www.acc.org' },
            { icon: '☕', name: 'BMJ Nutrition — Caffeine Research', url: 'https://nutrition.bmj.com' }
        ];
        ...
        sourceLinks.forEach(function (s) {
            ...
            sourcesHtml += '<span style="color:#00bbff;font-size:9px;opacity:0.7;">🔗 Truy cập</span>';
            sourcesHtml += '</a>';
        });
        sourcesHtml += '</div>';
    ...
    var verifiedB2bHtml = '<div style="background:rgba(0, 255, 128, 0.08); border: 1px solid rgba(0, 255, 128, 0.3); border-radius: 8px; padding: 10px 12px; margin-bottom: 12px; display: flex; align-items: center; gap: 10px; box-shadow: 0 2px 10px rgba(0,255,128,0.1);">' +
        '<div style="font-size: 24px; animation: pulse 2s infinite;">🛡️</div>' +
        '<div style="flex: 1;"><div style="color: #00ff80; font-weight: 900; font-size: 11px; letter-spacing: 1px; text-transform: uppercase;">MÔ HÌNH AI XÁC THỰC</div><div style="color: #c0d0c0; font-size: 10px; margin-top: 3px; line-height: 1.4;">Kết quả đã được đối chiếu & tinh chỉnh bằng thuật toán từ cơ sở dữ liệu bệnh án y khoa nội bộ <b>đã được mã hóa đa lớp</b>.</div></div>' +
        '</div>';

    var html = verifiedB2bHtml + '<div class="ai-result-grid">' +
